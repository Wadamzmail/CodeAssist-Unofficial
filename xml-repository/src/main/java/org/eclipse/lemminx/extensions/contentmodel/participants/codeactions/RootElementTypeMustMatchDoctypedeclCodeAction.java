 ...
 *
 * <p>This class depends on the diagnostic message (Diagnostic#message) to determine the correct
 * root name.
 *
 * <p>This class assumes that the diagnostic message is in this format: Document root element
 * "<current root name>", must match DOCTYPE root "<expected root name>".
 */
public class RootElementTypeMustMatchDoctypedeclCodeAction implements ICodeActionParticipant {
  ...
  /**
   * Returns the current root name, extracted from <code>message</code> The provided <code>message
   * </code> must match this format: ... root element "<current root name>", must match DOCTYPE ...
   *
  ...
  /**
   * Returns the doctype root name, extracted from <code>message</code> The provided <code>message
   * </code> must match this format: ... DOCTYPE root "<root name>".
   *
   * <p>TODO: This code is a workaround until this issue is fixed:
   * https://github.com/microsoft/language-server-protocol/issues/887
   *
