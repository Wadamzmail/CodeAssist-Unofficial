 ...
 * schema:
 *
 * <p><article></article> Error: element "article" incomplete; expected element "title", "title2" or
 * "titleOtherChoice"
 *
 * <p>To fix the error, there will three code actions generated to insert each option
 */
public class required_elements_missing_expectedCodeAction implements ICodeActionParticipant {
    ...
    try {
      ...
      for (CMDocument cmDocument : contentModelManager.findCMDocument(element)) {
        ...
        if (matchesCMElement != null) {
          ...
          for (CMElementDeclaration child :
              (((CMRelaxNGElementDeclaration) matchesCMElement).getPossibleRequiredElements())) {
            ...
            CodeAction insertElementCodeAction =
                createInsertMissingElementCodeAction(
                    child, element, insertStr, request, cancelChecker);
  ...
  private static CodeAction createInsertMissingElementCodeAction(
    ...
    if (request.canSupportResolve()) {
      JsonObject data =
          DataEntryField.createData(
              ...
              required_elements_missing_expectedCodeActionResolver.PARTICIPANT_ID);
      return insertMissingExpectedElementUnresolvedCodeAction(
          title, diagnostic, data, element.getLocalName());
    } else {
      return insertMissingExpectedElementCodeAction(
          document, title, domElement, insertStr, diagnostic);
    }
  }

  private static CodeAction insertMissingExpectedElementUnresolvedCodeAction(
      String title, Diagnostic diagnostic, JsonObject data, String elementName) {
  ...
  private static CodeAction insertMissingExpectedElementCodeAction(
