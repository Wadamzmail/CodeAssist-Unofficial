 ...
 *
 * <p>Given this XML where the expected child elements are not present as defined in the relaxNG
 * schema:
 *
 * <p><servlet></servlet> Error: Child elements are missing from element: - servlet
 *
 * <p>To fix the error, the code action will suggest inserting the expected elements inside the
 * parent tag
 */
public class required_element_missingCodeAction implements ICodeActionParticipant {
  ...
  public void doCodeAction(
    ...
    try {
      ...
      if (!request.canSupportResolve()) {
        insertStrRequired =
            generator.generateMissingElements(
                request.getComponent(ContentModelManager.class), element, true);
        insertStrAll =
            generator.generateMissingElements(
                request.getComponent(ContentModelManager.class), element, false);
      }
      CodeAction insertRequriedExpectedElementCodeAction =
          createInsertExpectedElementsCodeAction(
              true, element, insertStrRequired, request, cancelChecker);

      CodeAction insertAllExpectedElementCodeAction =
          createInsertExpectedElementsCodeAction(
              false, element, insertStrAll, request, cancelChecker);
  ...
  private static CodeAction createInsertExpectedElementsCodeAction(
    ...
    DOMDocument document = request.getDocument();
    String title =
        isGenerateRequired ? "Insert only required elements" : "Insert all expected elements";
    if (request.canSupportResolve()) {
      JsonObject data =
          DataEntryField.createData(
              document.getDocumentURI(), required_element_missingCodeActionResolver.PARTICIPANT_ID);
      return insertExpectedElementsUnresolvedCodeAction(
          title, diagnostic, data, isGenerateRequired);
    } else {
      return insertExpectedElementCodeAction(document, title, domElement, insertStr, diagnostic);
    }
  }

  private static CodeAction insertExpectedElementsUnresolvedCodeAction(
      String title, Diagnostic diagnostic, JsonObject data, boolean isGenerateRequired) {
  ...
  private static CodeAction insertExpectedElementCodeAction(
      ...
      throws BadLocationException {
    Position childElementPositionStartTag =
        document.positionAt(element.getStartTagCloseOffset() + 1);
    ...
    return CodeActionFactory.replace(
        title, targetRange, insertStr, document.getTextDocument(), diagnostic);
  }
}
