...
public class OpenQuoteExpectedCodeAction implements ICodeActionParticipant {
  ...
  private static void insertQuotationForAttr(
      DOMNode attr,
    ...
    if (possibleEndPosition != null && possibleValue != null) {
      removeContentAction =
          CodeActionFactory.replace(
              "Insert quotations",
              new Range(codeactionPosition, possibleEndPosition),
              q + possibleValue + q,
  ...
  private static void insertQuotationForParameter(
    ...
    SharedSettings sharedSettings = request.getSharedSettings();
    String q = sharedSettings.getPreferences().getQuotationAsString();
    CodeAction insertQuotationsAction =
        CodeActionFactory.replace(
            "Insert quotations",
            parameter.getTargetRange(),
