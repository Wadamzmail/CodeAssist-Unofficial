...
public class XSDRenameParticipant implements IRenameParticipant {
  ...
  public Either<Range, PrepareRenameResult> prepareRename(
      IPrepareRenameRequest request, CancelChecker cancelChecker) {
  ...
  public void doRename(
      IRenameRequest request, IRenameResponse renameResponse, CancelChecker cancelChecker) {
    renameResponse.addTextDocumentEdit(getRenameTextDocumentEdit(request, cancelChecker));
  }

  private TextDocumentEdit getRenameTextDocumentEdit(
      IRenameRequest request, CancelChecker cancelChecker) {
    // XSD rename can be applied for:
    // - xsd:complexType/@name
    // - xs:simpleType/@name
    DOMAttr attr = findAttrToRename(request);
    ...
    return creatTextDocumentEdit(
        document, renameAttributeValueTextEdits(document, attr, newText, locations));
  }
  ...
  private List<TextEdit> renameAttributeValueTextEdits(
      DOMDocument document, DOMAttr attribute, String newText, List<Location> locations) {
  ...
  private String getAttrTextValueFromPosition(DOMDocument document, Position position)
  ...
  /**
   * Returns the xsd:complexType/@name or xs:simpleType/@name to rename or null otherwise.
   *
   * @param request the position request.
   * @return the xsd:complexType/@name or xs:simpleType/@name to rename or null otherwise.
   */
