...
public class XML2RelaxNGGenerator extends AbstractXML2GrammarGenerator<RelaxNGGeneratorSettings> {
  ...
  protected void generate(
    ...
    for (ElementDeclaration element : grammar.getElements()) {
      ...
      generateRngElement(
          ...
          element,
          settings,
          rootElementName.equals(element.getName()),
  ...
  private void generateRngElement(
      XMLBuilder schema,
      String prefix,
      ElementDeclaration elementDecl,
      RelaxNGGeneratorSettings settings,
      boolean isRoot,
      String parentPatternName,
      CancelChecker cancelChecker) {
    ...
    if (!hasChildren && !hasAttributes) {
      ...
      if (hasChildren) {
        ...
        for (ElementDeclaration child : children) {
          generateRngElement(
              schema, prefix, child, settings, false, child.getName() + "Content", cancelChecker);
        }
  ...
  private void generateAttribute(
      XMLBuilder schema,
      Collection<AttributeDeclaration> attributes,
      String prefix,
      RelaxNGGeneratorSettings settings,
      CancelChecker cancelChecker) {
