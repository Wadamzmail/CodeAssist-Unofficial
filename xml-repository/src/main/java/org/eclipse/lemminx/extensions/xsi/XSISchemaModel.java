...
public class XSISchemaModel {
  ...
  public static final String TYPE_DOC =
      "Specifies the type of an element. This attribute labels an element as a particular type,"
          + " even though there might not be an element declaration in the schema binding that"
          + " element to the type.";
  public static final String NIL_DOC =
      "Indicates if an element should contain content. Valid values are `true` or `false`";
  ...
  public static final String NO_NAMESPACE_SCHEMA_LOCATION_DOC =
      "The xsi:noNamespaceSchemaLocation attribute can be used in an XML document "
          + lineSeparator
  ...
  public static final String XSI_DOC =
      "The namespace that defines important attributes such as `noNamespaceSchemaLocation` and"
          + " `schemaLocation`.";

  public static void computeCompletionResponses(
    ...
    if (inRootElement) {
      if (!hasAttribute(elementAtOffset, "xmlns")
          && !response.hasAttribute("xmlns")) { // "xmlns" completion
  ...
  private static void createCompletionItem(
      String attrName,
  ...
  public static void computeValueCompletionResponses(
      ICompletionRequest request, ICompletionResponse response, DOMDocument document)
      throws BadLocationException {
    Range editRange = request.getReplaceRange();
    int offset = document.offsetAt(editRange.getStart());
    ...
    if (attrName != null) {
      if (actualPrefix != null && attrName.equals(actualPrefix + ":nil")) {
        ...
        createCompletionItemsForValues(StringUtils.TRUE_FALSE_ARRAY, document, request, response);
          ...
          && document.getDocumentElement().equals(nodeAtOffset)) {
  ...
  private static void createSingleCompletionItemForValue(
      String value,
      DOMDocument document,
      ICompletionRequest request,
      ICompletionResponse response) {
  ...
  private static void createCompletionItemsForValues(
      Collection<String> enumerationValues,
      DOMDocument document,
      ICompletionRequest request,
      ICompletionResponse response) {
  ...
  /**
   * Checks if a given root has an attribute with the name: {@code prefix:suffix}. If no prefix
   * exists, put the name in {@code suffix}
   */
