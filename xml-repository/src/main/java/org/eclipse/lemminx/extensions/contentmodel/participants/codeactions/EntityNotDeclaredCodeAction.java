 ...
 *
 * <p>Precondition: {@link Diagnostic#getMessage()} should be in this format:
 *
 * <p>"The entity \"ENTITY NAME\" was referenced, but not declared."
 */
public class EntityNotDeclaredCodeAction implements ICodeActionParticipant {

  private static final Logger LOGGER =
      Logger.getLogger(EntityNotDeclaredCodeAction.class.getName());
  ...
  public void doCodeAction(
    ...
    try {
      ...
      if (docType != null) {
        ...
        // <!ENTITY nbsp "entity-value">
        // ]>
        addDoctypeAndEntityCodeAction(
            entityName, diagnostic, document, sharedSettings, codeActions);
      }
    } catch (BadLocationException e) {
      LOGGER.log(
          Level.SEVERE,
          "In EntityNotDeclaredCodeAction the DOMDocument offset(s) is at a BadLocation",
          e);
    }
  }

  /**
   * Add a code action that inserts entity declaration to the current DOCTYPE's internal subset
   *
   * <p>If the internal subset does not exist, the code action inserts it as well
   *
   ...
   */
  private void addEntityCodeAction(
      String entityName,
  ...
  /**
   * Add a code action that inserts the DOCTYPE declaration containing an internal subset with an
   * entity declaration
   *
   * @param entityName the entity name for the entity declaration
   * @param diagnostic the code action's diagnostic
   ...
   * @throws BadLocationException
   */
  private void addDoctypeAndEntityCodeAction(
      String entityName,
      Diagnostic diagnostic,
      DOMDocument document,
      SharedSettings settings,
   ...
   *
   * <p>TODO: This code is a workaround until this issue is fixed:
   ...
   * @throws BadLocationException
   */
  private static String getEntityName(Diagnostic diagnostic, DOMDocument doc)
      throws BadLocationException {
    Range range = diagnostic.getRange();
    String name =
        doc.getText().substring(doc.offsetAt(range.getStart()), doc.offsetAt(range.getEnd()));
