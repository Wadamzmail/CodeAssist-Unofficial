...
public class XMLCatalogDocumentLinkParticipant implements IDocumentLinkParticipant {
  ...
  private static Logger LOGGER =
      Logger.getLogger(XMLCatalogDocumentLinkParticipant.class.getName());
   ...
   * @param catalogEntry The CatalogEntry that should be turned into a DocumentLink
   * @return a document link that links the CatalogEntry to the external document it references or
   *     null otherwise
   */
  private static DocumentLink createDocumentLinkFromCatalogEntry(
      DOMDocument document, CatalogEntry catalogEntry) {
    try {
