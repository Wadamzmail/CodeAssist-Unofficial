...
public class AttributeNameCompletionResolver extends AbstractAttributeCompletionResolver {
    ...
    try {
      ...
      for (CMDocument cmDocument : cmDocuments) {
        CMElementDeclaration cmElement =
            cmDocument.findCMElement(element, element.getNamespaceURI());
        if (cmElement != null) {
          MarkupContent documentation =
              getDocumentationForAttributeValue(cmElement, attributeName, request);
  ...
  private static MarkupContent getDocumentationForAttributeValue(
      CMElementDeclaration cmElement,
      String attributeName,
      ICompletionItemResolverRequest request) {
    CMAttributeDeclaration cmAttribute = cmElement.findCMAttribute(attributeName, null);
    if (cmAttribute != null) {
      return XMLGenerator.createMarkupContent(cmAttribute, cmElement, request);
    }
    return null;
  }
}
