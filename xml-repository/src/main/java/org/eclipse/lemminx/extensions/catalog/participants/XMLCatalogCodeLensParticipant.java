 ...
 *
 * <p>When clicked, this catalog runs the client command <code>"xml.update.configuration"</code> to
 * update the workspace settings to register the catalog for use. The client needs to implement the
 * <code>"xml.update.configuration"</code> command in order for this to work.
 */
public class XMLCatalogCodeLensParticipant implements ICodeLensParticipant {
  ...
  private void createRegisterCatalogLenses(ICodeLensRequest request, List<CodeLens> lenses) {
    ...
    if (catalogs == null || !Arrays.asList(catalogs).contains(documentURI)) {
      ...
      // [Register Catalog]
      // <catalog ...>
      ConfigurationItemEdit configurationItemEdit =
          new ConfigurationItemEdit(
              "xml.catalogs",
              documentURI,
              ConfigurationItemEditType.Add,
              ConfigurationItemValueKind.File);

      Command command =
          new Command(
              "Register Catalog",
              ClientCommands.UPDATE_CONFIGURATION,
      ...
      ConfigurationItemEdit configurationItemEdit =
          new ConfigurationItemEdit(
              "xml.catalogs",
              documentURI,
              ConfigurationItemEditType.Delete,
              ConfigurationItemValueKind.File);
      Command command =
          new Command(
              "Unregister Catalog",
