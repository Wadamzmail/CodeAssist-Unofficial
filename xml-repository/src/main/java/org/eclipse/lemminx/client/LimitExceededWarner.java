 ...
 *
 * <p>Cache is a Map<String, Set<String>>
 *
 * <p>where the keys are the feature names and the values are the file paths for which notifications
...
public class LimitExceededWarner extends AbstractXMLNotifier {
   ...
   * <code>feature</code>'s cache set
   *
   * <p>After sending the warning, the uri is added to the <code>feature</code>'s cache set
   *
   ...
   * cache
   *
   * <p>After sending the warning, the uri is added to the cache
   *
   * @param uri the file uri
  ...
  private void sendLimitExceededWarning(String uri, int resultLimit, LimitFeature feature) {
    ...
    message +=
        "For performance reasons, "
            ...
            + resultLimit
            + " items.\nIf a new limit is set, please close and reopen this file to recompute "
            + feature.getName()
            + ".";
