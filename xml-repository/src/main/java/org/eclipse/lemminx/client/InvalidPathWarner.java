 ...
 *
 * <p>The cache is a Map<String, Set<String>>
 *
 * <p>where the keys are the feature names and the value is a set of file paths for which
 * notifications have been sent already
 */
public class InvalidPathWarner extends AbstractXMLNotifier {
  ...
  /**
   * Sends the invalid path warning to the client only if the provided <code>invalidPaths</code> set
   * does not match the <code>feature</code>'s cache set
   *
   * <p>After sending the warning, the <code>feature</code>'s cache set is replaced with the
   * provided <code>invalidPaths</code> set
   *
