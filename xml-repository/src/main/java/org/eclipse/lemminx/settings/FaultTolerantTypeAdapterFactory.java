...
/**
 * Creates TypeAdapters that are wrapped in a try/catch statement, so if anything goes wrong when
 * deserializing an object, it is set to null and not parsed.
 */
public class FaultTolerantTypeAdapterFactory implements TypeAdapterFactory {

  private static final Logger LOGGER =
      Logger.getLogger(FaultTolerantTypeAdapterFactory.class.getName());
  ...
  /**
   * Wrap the reader of the given TypeAdapter with a try/catch, and skip reading the value if an
   * error occurs
   *
    ...
    return new TypeAdapter<T>() {
      ...
      public T read(JsonReader in) throws IOException {
        try {
          ...
          LOGGER.warning(
              "Encountered an invalid setting. Using the default value. "
                  + //
