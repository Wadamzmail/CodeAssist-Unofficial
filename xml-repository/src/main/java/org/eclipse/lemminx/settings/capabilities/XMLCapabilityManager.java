/**
 * Copyright (c) 2018 Red Hat, Inc. and others. All rights reserved. This program and the
 ...
 *
 * <p>A capability is a service (Formatting, Highlighting, ...) that the server is able to provide.
 * This server will tell the client about the services it is capable of.
 */
public class XMLCapabilityManager {
  ...
  public XMLCapabilityManager(
      LanguageClient languageClient, XMLTextDocumentService textDocumentService) {
    this.languageClient = languageClient;
    this.textDocumentService = textDocumentService;
  }

  /**
   * Creates and sets a {@link ClientCapabilitiesWrapper} instance formed from clientCapabilities
   *
  ...
  public void setClientCapabilities(
    ...
    this.clientWrapper =
        new ClientCapabilitiesWrapper(clientCapabilities, extendedClientCapabilities);
  }
  ...
  /**
   * Registers all dynamic capabilities that the server does not support client side preferences
   * turning on/off
   */
  ...
  public void registerExecuteCommand(List<String> commands) {
    registerCapability(
        WORKSPACE_EXECUTE_COMMAND_ID,
        WORKSPACE_EXECUTE_COMMAND,
        new ExecuteCommandOptions(commands));
  }

  /**
   * Registers(indicates the servers ability to support the service) all capabilities that have the
   * ability to be turned on/off on the client side through preferences.
   *
   * <p>In the case the preference is set to off/false this server will tell the cliet it does not
   * support this capability.
   *
   * <p>If a capability is not dynamic, it's handled by {@link ServerCapabilitiesInitializer}
   */
  public void syncDynamicCapabilitiesWithPreferences() {
    XMLFormattingOptions formattingPreferences =
        this.textDocumentService.getSharedFormattingSettings();

    if (this.getClientCapabilities().isFormattingDynamicRegistrationSupported()) {
      toggleCapability(
          formattingPreferences.isEnabled(),
          FORMATTING_ID,
          ServerCapabilitiesConstants.TEXT_DOCUMENT_FORMATTING,
          null);
    }
    ...
    if (this.getClientCapabilities().isRangeFormattingDynamicRegistrationSupported()) {
      toggleCapability(
          formattingPreferences.isEnabled(),
          FORMATTING_RANGE_ID,
          ServerCapabilitiesConstants.TEXT_DOCUMENT_RANGE_FORMATTING,
          null);
    }
    ...
    if (this.getClientCapabilities().isDocumentSymbolDynamicRegistrationSupported()) {
      toggleCapability(
          symbolSettings.isEnabled(), DOCUMENT_SYMBOL_ID, TEXT_DOCUMENT_DOCUMENT_SYMBOL, null);
    }
