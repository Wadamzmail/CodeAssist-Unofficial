...
/**
 * This class is the root of all formatting settings. It is necessary to update this class for any
 * new additions.
 *
 * <p>All defaults should be set here to eventually be overridden if needed.
 */
public class XMLFormattingOptions extends org.eclipse.lemminx.settings.LSPFormattingOptions {
  ...
  public static final String DEFAULT_XSI_SCHEMA_LOCATION_SPLIT =
      XSISchemaLocationSplit.onPair.name();
   ...
   *
   * <ul>
   *   <li>{@link #expand} : expand empty elements. With this option the following XML:
   *       <pre>{@code
   * <example />
   * }</pre>
   ...
   * <example><example>
   * }</pre>
   *   <li>{@link #collapse} : collapse empty elements. With this option the following XML:
   *       <pre>{@code
   * <example></example>
   * }</pre>
   *       will be formatted to :
  ...
  /**
   * Create an XMLFormattingOptions instance with the option to initialize default values for all
   * supported settings.
   */
  ...
  public XMLFormattingOptions(
      int tabSize, boolean insertSpaces, boolean initializeDefaultSettings) {
   ...
   *
   * @param legacy true if the legacy formatter must be used and false otherwise.
   */
   ...
   *
   * <p>A setting for enabling the XML formatter to move the closing bracket of a tag with at least
   * 2 attributes to a new line.
   *
