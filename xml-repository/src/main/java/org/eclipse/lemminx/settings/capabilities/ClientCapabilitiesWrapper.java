...
public class ClientCapabilitiesWrapper {
  ...
  public ClientCapabilitiesWrapper(
      ClientCapabilities capabilities, ExtendedClientCapabilities extendedCapabilities) {
   ...
   * IMPORTANT
   *
   * <p>This should be up to date with all Server supported capabilities
   */
  ...
  public boolean isDidChangeWatchedFilesRegistered() {
    return v3Supported
        && isDynamicRegistrationSupported(capabilities.getWorkspace().getDidChangeWatchedFiles());
  }
  ...
  private boolean isDynamicRegistrationSupported(DynamicRegistrationCapabilities capability) {
    return capability != null
        && capability.getDynamicRegistration() != null
