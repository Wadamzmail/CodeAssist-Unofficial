...
public class CompletionItemDefaultsUtils {
  ...
  public static void process(CompletionResponse completionResponse, SharedSettings sharedSettings) {
    ...
    if (sharedSettings
        ...
        .isCompletionListItemDefaultsSupport(ITEM_DEFAULTS_EDIT_RANGE)) {
  ...
  /**
   * Sets the most common editRange in the completion list in item defaults and processes the
   * completion list.
   *
   * @param completionList the completion list
   * @param itemDefaults the item defaults
   */
  private static void setToMostCommonEditRange(
      List<CompletionItem> completionList, CompletionItemDefaults itemDefaults) {
    Map<Range, List<CompletionItem>> itemsByRange =
    ...
    completionList =
        itemsByRange.values().stream().flatMap(Collection::stream).collect(Collectors.toList());
  }

  /**
   * Sets the most common InsertTextFormat in the completion list in item defaults and processes the
   * completion list.
   *
   * @param completionList the completion list
   * @param itemDefaults the item defaults
   */
  private static void setToMostCommonInsertTextFormat(
      List<CompletionItem> completionList, CompletionItemDefaults itemDefaults) {
    Map<InsertTextFormat, List<CompletionItem>> itemsByInsertTextFormat =
        completionList.stream()
    ...
    for (Map.Entry<InsertTextFormat, List<CompletionItem>> entry :
        itemsByInsertTextFormat.entrySet()) {
      int currentSize = entry.getValue().size();
      if (currentSize > maxCount) {
        maxCount = currentSize;
    ...
    itemsByInsertTextFormat
        .get(mostCommonInsertTextFormat)
        .forEach(
            item -> {
    ...
    completionList =
        itemsByInsertTextFormat.values().stream()
