...
public class ErrorMessages {
  ...
  public static final String UNDEFINED_VARIABLE =
      "Could not find identifier '%s'\n"
          + "\n"
          + "Check that the identifier is spelled correctly, and that no <import> or <variable>"
          + " tags are missing.";

  public static final String CANNOT_FIND_SETTER_CALL =
      "Cannot find a setter for <%s %s> that accepts parameter type '%s'\n"
          + "\n"
          + "If a binding adapter provides the setter, check that the adapter is annotated"
          + " correctly and that the parameter type matches.";
  ...
  public static final String ROOT_TAG_NOT_SUPPORTED =
      "You must target API level 14 or greater to support 'android:tag' on root elements of data"
          + " bound layouts";
  ...
  public static final String CANNOT_FIND_GETTER_CALL =
      "Cannot find a getter for <%s %s> that accepts parameter type '%s'\n"
          + "\n"
          + "If a binding adapter provides the getter, check that the adapter is annotated"
          + " correctly and that the parameter type matches.";
  ...
  public static final String TWO_WAY_EVENT_ATTRIBUTE =
      "The attribute '%s' is generated and reserved for two-way data binding so an expression"
          + " cannot be assigned to it";

  public static final String CANNOT_FIND_ABSTRACT_METHOD =
      "Cannot assign callback expression to '%s'\n"
          + "\n"
          + "Make sure you aren't using lambda syntax if the expression should only return a value"
          + " directly";

  public static final String CALLBACK_ARGUMENT_COUNT_MISMATCH =
      "Number of lambda parameters is incorrect\n"
          + "\n"
          + "'%s::%s' accepts %d parameter(s), but the assigned expression uses %d parameter(s)."
          + " The expression should have no parameters or an equal number of parameters.";
  ...
  public static final String RECURSIVE_OBSERVABLE =
      "Observable fields (LiveData, Observable etc) cannot contain a value type of themselves: %s"
          + " .\n"
          + "\n"
          + "This would create a situation where data binding would need to unwrap an observable"
          + " indefinitely.(e.g. unwrapping a class like `Foo extends Observable<Foo>` would result"
          + " into another `Foo`)";
  ...
  public static final String UNEXPECTED_ERROR_IN_LAYOUT =
      "Unexpected error while processing layout file: %s.xml\n"
          ...
          + BUGANIZER_URL
          + " with a sample project that reproduces the problem.";
}
