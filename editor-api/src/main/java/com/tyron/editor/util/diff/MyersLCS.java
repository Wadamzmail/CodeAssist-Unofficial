...
/**
 * Algorithm for finding the longest common subsequence of two strings Based on E.W. Myers / An
 * O(ND) Difference Algorithm and Its Variations / 1986 O(ND) runtime, O(N) memory
 *
 * <p>Created by Anton Bannykh
 */
class MyersLCS {
   ...
   *
   * <p>Under certains assumptions about the distribution of the elements of the sequences the
   * expected running time of the myers algorithm is O(N + D^2). Thus under given constraints it
   * reduces to O(N).
   */
  ...
  public void executeWithThreshold() throws FilesTooBigForDiffException {
    int threshold =
        Math.max(
            20000 + 10 * (int) Math.sqrt(myCount1 + myCount2), DiffConfig.DELTA_THRESHOLD_SIZE);
  ...
  private void execute(
    ...
    if (oldStart < oldEnd && newStart < newEnd) {
      ...
      for (int d = 0; d <= halfD; ++d) {
        ...
        for (int k = L; k <= R; k += 2) {
          int x =
              k == L || k != R && VForward[k - 1] < VForward[k + 1]
                  ? VForward[k + 1]
                  : VForward[k - 1] + 1;
          int y = x - k + newLength;
          x +=
              commonSubsequenceLengthForward(
                  oldStart + x,
                  newStart + y,
        ...
        for (int k = L; k <= R; k += 2) {
          int x =
              k == L || k != R && VBackward[k - 1] < VBackward[k + 1]
                  ? VBackward[k + 1]
                  : VBackward[k - 1] + 1;
          int y = x - k + newLength;
          x +=
              commonSubsequenceLengthBackward(
                  oldEnd - 1 - x,
                  newEnd - 1 - y,
                  Math.min(oldEnd - oldStart - x, newEnd - newStart - y));
          VBackward[k] = x;
      ...
      if (td > 1) {
        ...
        while (x < oldEnd && y < newEnd) {
          final int commonLength =
              commonSubsequenceLengthForward(x, y, Math.min(oldEnd - x, newEnd - y));
          if (commonLength > 0) {
