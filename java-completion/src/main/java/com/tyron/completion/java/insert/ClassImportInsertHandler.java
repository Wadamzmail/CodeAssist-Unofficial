...
/**
 * Handles the proper insertion of import declarations when a class name has been selected from the
 * completion list.
 */
public class ClassImportInsertHandler extends DefaultInsertHandler {
  ...
  public ClassImportInsertHandler(JavacUtilitiesProvider provider, File file, CompletionItem item) {
    super(item);
    this.file = file;
    this.utils = provider;
  }
