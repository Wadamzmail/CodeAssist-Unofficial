...
public class SmartClassNameCompletionProvider extends BaseCompletionProvider {
  ...
  public void complete(
    ...
    List<String> fullyQualifiedNames =
        ClassNameCompletionProvider.getFullyQualifiedNames(
            path.getCompilationUnit(), partial, task, true);
      ...
      if (assignable) {
        ...
        item.setInsertHandler(
            new ClassImportInsertHandler(
                task, new File(path.getCompilationUnit().getSourceFile().toUri()), item));
