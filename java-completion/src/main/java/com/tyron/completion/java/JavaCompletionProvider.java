...
public class JavaCompletionProvider extends CompletionProvider {
  ...
  public CompletionList complete(CompletionParameters params) {
    ...
    try {
      ...
      IdeLog.getCurrentLogger(getClass())
          .severe("Failed to complete: " + Throwables.getStackTraceAsString(t));
    }
    if (complete == null) {
      return CompletionList.EMPTY;
    }
  ...
  public CompletionList.Builder completeV2(CompletionParameters parameters) {
    CompilationInfo compilationInfo =
        CompilationInfo.get(parameters.getProject(), parameters.getFile());
    ...
    SimpleJavaFileObject fileObject =
        new SimpleJavaFileObject(parameters.getFile().toURI(), JavaFileObject.Kind.SOURCE) {
          @Override
          public CharSequence getCharContent(boolean ignoreEncodingErrors) {
            StringBuilder pruned =
                new StringBuilder(
    ...
    switch (scanned.getLeaf().getKind()) {
      case IDENTIFIER:
        ...
        //                    // scan the tree before the current one, check if its a "new"
        // expression
        //                    TreePath previousPath = new FindCurrentPath(javacTask).scan(unit,
        // parameters.getIndex() - 1);
        //
        //                    // we are in a new expression
        //                    // e.g. List list = new ...
        //                    if (previousPath != null && previousPath.getLeaf() instanceof
        // NewClassTree) {
        //                        // now suggest types that are applicable to the current type
        //                        new SmartClassNameCompletionProvider(null).complete(builder,
        // javacUtilities,
        //                                scanned, parameters.getPrefix(), false);
        //                    }
        //                }
        new IdentifierCompletionProvider(null)
            .complete(builder, javacUtilities, scanned, parameters.getPrefix(), false);
        break;
      case MEMBER_SELECT:
        new MemberSelectCompletionProvider(null)
            .complete(builder, javacUtilities, scanned, parameters.getPrefix(), false);
        break;
