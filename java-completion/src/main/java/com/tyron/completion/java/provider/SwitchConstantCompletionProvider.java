...
public class SwitchConstantCompletionProvider extends BaseCompletionProvider {
  ...
  public static void completeSwitchConstant(
      CompletionList.Builder builder, JavacUtilitiesProvider task, TreePath path, String partial) {
    checkCanceled();
    ...
    TypeMirror type = task.getTrees().getTypeMirror(path);
    ...
    for (Element member : task.getElements().getAllMembers(element)) {
