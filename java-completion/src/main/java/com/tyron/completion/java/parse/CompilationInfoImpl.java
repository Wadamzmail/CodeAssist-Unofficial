...
public class CompilationInfoImpl {
  ...
  /**
   * Returns {@link JavacTaskImpl}, when it doesn't exist it's created.
   *
  ...
  public List<Diagnostic<? extends JavaFileObject>> getDiagnostics(JavaFileObject fileObject) {
    DiagnosticListenerImpl.Diagnostics errors =
        ((DiagnosticListenerImpl) diagnosticListener).getErrors(fileObject);
    List<Diagnostic<? extends JavaFileObject>> partialReparseErrors =
        ((DiagnosticListenerImpl) diagnosticListener).partialReparseErrors;
    List<Diagnostic<? extends JavaFileObject>> affectedErrors =
        ((DiagnosticListenerImpl) diagnosticListener).affectedErrors;
    int errorsSize = 0;
    ...
    List<Diagnostic<? extends JavaFileObject>> localErrors =
        new ArrayList<>(
            errorsSize
