...
public class ClassNameCompletionProvider extends BaseCompletionProvider {
  ...
  public static void addClassNames(
    ...
    List<String> fullyQualifiedNames = getFullyQualifiedNames(root, partial, task, caseSensitive);
    for (String className : fullyQualifiedNames) {
      CompletionItem item = classItem(className);
      item.data = className;
      item.setInsertHandler(
          new ClassImportInsertHandler(task, new File(root.getSourceFile().toUri()), item));
      item.setSortText(JavaSortCategory.TO_IMPORT.toString());
      list.addItem(item);
    }
  }

  public static List<String> getFullyQualifiedNames(
      CompilationUnitTree root,
      String partial,
      JavacUtilitiesProvider task,
      boolean caseSensitive) {
