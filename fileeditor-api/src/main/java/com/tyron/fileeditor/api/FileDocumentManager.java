...
public abstract class FileDocumentManager {
   ...
   * are converted into the ones used normally on the system, or the ones explicitly specified by
   * the user. Encoding settings are honored.
   *
   * <p>Should be invoked on the event dispatch thread.
   */
   ...
   * trailing spaces on Save' functionality). When saving, {@code \n} line separators are converted
   * into the ones used normally on the system, or the ones explicitly specified by the user.
   * Encoding settings are honored.
   *
   * <p>Should be invoked on the event dispatch thread.
   *
