...
public class ResourceSet extends DataSet<ResourceMergerItem, ResourceFile> {
  ...
  protected ResourceFile createFileAndItemsFromXml(@NonNull File file, @NonNull Node fileNode)
    ...
    if (NodeUtils.getAttribute(fileNode, SdkConstants.ATTR_PREPROCESSING) != null) {
      ...
      ResourceMergerItem item =
          new ResourceMergerItem(nameAttr, mNamespace, type, null, mIsFromDependency, mLibraryName);
      return new ResourceFile(file, item, folderConfiguration);
    }
  }

  @Override
  protected void readSourceFolder(File sourceFolder, ILogger logger, DocumentBuilderFactory factory)
      throws MergingException {
  ...
  private void doHandleChangedFile(
      @NonNull File changedFile, ResourceFile resourceFile, @NonNull DocumentBuilderFactory factory)
      throws MergingException {
    switch (resourceFile.getType()) {
      ...
      case GENERATED_FILES:
        handleChangedItems(resourceFile, getResourceMergerItemsForGeneratedFiles(changedFile));
        break;
      case XML_VALUES:
  ...
  private void handleChangedItems(ResourceFile resourceFile, List<ResourceMergerItem> currentItems)
      throws MergingException {
  ...
  /**
   * Reads the content of a typed resource folder (sub folder to the root of res folder), and loads
  ...
  private void processNewResourceFile(File sourceFolder, ResourceFile resourceFile)
    ...
    if (resourceFile != null) {
      if (resourceFile.getType() == DataFile.FileType.GENERATED_FILES && mGeneratedSet != null) {
  ...
  private ResourceFile createResourceFile(
    ...
    if (folderData.type != null) {
      ...
      if (!generatedFiles.isEmpty()) {
        return ResourceFile.generatedFiles(file, generatedFiles, folderData.folderConfiguration);
      } else if (mShouldParseResourceIds
          && folderData.isIdGenerating
      ...
      try {
        ValueResourceParser2 parser = new ValueResourceParser2(file, mNamespace, mLibraryName);
        parser.setTrackSourcePositions(mTrackSourcePositions);
        parser.setCheckDuplicates(mCheckDuplicates);
  ...
  private List<ResourceMergerItem> getResourceMergerItemsForGeneratedFiles(@NonNull File file)
    ...
    for (File generatedFile : filesToBeGenerated) {
      FolderData generatedFileFolderData = getFolderData(generatedFile.getParentFile());
   ...
   * @return the FolderData object, or null if we can't determine the {#link ResourceFolderType} of
   *     the folder.
   */
  @Nullable
  ...
  void appendToXml(
    ...
    if (mGeneratedSet != null) {
      NodeUtils.addAttribute(
          document, setNode, null, ATTR_GENERATED_SET, mGeneratedSet.getConfigName());
    }

    if (mIsFromDependency) {
      NodeUtils.addAttribute(
          document, setNode, null, ATTR_FROM_DEPENDENCY, SdkConstants.VALUE_TRUE);
    }
