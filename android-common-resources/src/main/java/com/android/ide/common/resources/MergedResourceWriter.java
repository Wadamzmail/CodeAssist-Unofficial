...
public class MergedResourceWriter
  ...
  @Nullable private MergingLog mMergingLog;
   ...
   * values files that must be written out even with no touched or updated resources, in case one or
   * more resources were removed.
   */
  private Set<String> mQualifierWithDeletedValues;

  /** Futures we are waiting for... */
  @NonNull private final ConcurrentLinkedDeque<Future<File>> mCompiling;

  /** File where {@link #mCompiledFileMap} is read from and where its contents are written. */
  @NonNull private final File mCompiledFileMapFile;

  /**
   * Maps resource files to their compiled files. Used to compiled resources that no longer exist.
   */
  ...
  public static MergedResourceWriter createWriterWithoutPngCruncher(
    ...
    return new MergedResourceWriter(
        new MergedResourceWriterRequest(
            ...
            rootFolder,
            publicFile,
            blameLogFolder != null ? new MergingLog(blameLogFolder, moduleSourceSet) : null,
            preprocessor,
    ...
    try {
      ...
      while (!mCompileResourceRequests.isEmpty()) {
        ...
        try {
          ...
          if (mMergingLog != null) {
            File destination =
                mergeWriterRequest.getResourceCompilationService().compileOutputFor(request);
            mMergingLog.logCopy(
                fileToCompile,
          ...
          if (!mergeWriterRequest.getModuleSourceSets().isEmpty()) {
            compileResourceRequest.useRelativeSourcePath(mergeWriterRequest.getModuleSourceSets());
          }

          mergeWriterRequest.getResourceCompilationService().submitCompile(compileResourceRequest);
    ...
    if (type == ResourceFile.FileType.XML_VALUES) {
      ...
      if (item.isTouched()) {
        ...
        if (type == DataFile.FileType.GENERATED_FILES) {
          try {
            FileGenerationParameters workItem =
                new FileGenerationParameters(item, mergeWriterRequest.getPreprocessor());
        ...
        CompileResourceRequest crunchRequest =
            new CompileResourceRequest(file, getRootFolder(), folderName, item.mIsFromDependency);
        if (!mergeWriterRequest.getModuleSourceSets().isEmpty()) {
    ...
    public void run() {
      try {
        workItem.resourcePreprocessor.generateFile(
            workItem.resourceItem.getFile(), workItem.resourceItem.getSourceFile().getFile());
      } catch (Exception e) {
  ...
  public void removeItem(
    ...
    ResourceFile.FileType replacedType = replacedBy != null ? replacedBy.getSourceType() : null;
  ...
  protected void postWriteAction() throws ConsumerException {
     ...
     */
    File tmpDir =
        new File(mergeWriterRequest.getTemporaryDirectory(), SdkConstants.FD_MERGED_DOT_DIR);
    try {
      FileUtils.cleanOutputDir(tmpDir);
    } catch (IOException e) {
      throw new ConsumerException(e);
        ...
        try {
          ...
          for (ResourceMergerItem item : items) {
            ...
            if (source != null) {
              ...
              XmlUtils.attachSourceFile(adoptedNode, new SourceFile(source.getFile()));
            }
          ...
          if (mergeWriterRequest.getNotCompiledOutputDirectory() != null) {
            File typeDir = new File(mergeWriterRequest.getNotCompiledOutputDirectory(), folderName);
            FileUtils.mkdirs(typeDir);
          ...
          if (publicNodes != null && mergeWriterRequest.getPublicFile() != null) {
            ...
            Files.asCharSink(mergeWriterRequest.getPublicFile(), Charsets.UTF_8).write(text);
          }
        } catch (Exception e) {
          throw new ConsumerException(e);
        }
  ...
  /**
   * Removes possibly existing layout file from the data binding output folder. If the original file
   * was a layout XML file it is possible that it contained data binding and was put into the data
   * binding layout output folder for data binding tasks to process.
   *
   ...
   *
   * @param resourceItem the source item that created the file to remove, this item must have a file
   *     associated with it
