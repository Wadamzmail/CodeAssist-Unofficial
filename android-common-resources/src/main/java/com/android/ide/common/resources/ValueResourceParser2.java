...
class ValueResourceParser2 {
  ...
  ValueResourceParser2(
      @NonNull File file, @NonNull ResourceNamespace namespace, @Nullable String libraryName) {
    mFile = file;
  ...
  static ResourceType getType(@NonNull Node node, @Nullable File from) throws MergingException {
    ...
    if (result != null) {
      ...
      throw MergingException.withMessage("Can't determine type for tag '%s'", XmlUtils.toXml(node))
          .withFile(from)
          .build();
    }
  }
  ...
  static Document parseDocument(
    ...
    try {
      if (trackPositions) {
        return PositionXmlParser.parse(new BufferedInputStream(new FileInputStream(file)), factory);
      } else {
  ...
  private static void checkDuplicate(
    ...
    if (!set.add(name) && resource.getType() != ResourceType.PUBLIC) {
      throw MergingException.withMessage(
              "Found item %s/%s more than one time", resource.getType().getDisplayName(), name)
          .withFile(from)
          .build();
    }
  }
