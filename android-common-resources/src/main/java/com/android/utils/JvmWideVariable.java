...
public final class JvmWideVariable<T> {
  ...
  /**
   * Creates a {@code JvmWideVariable} instance that can access a JVM-wide variable. If the JVM-wide
   * variable does not yet exist, this constructor creates the variable and initializes it with an
   * initial value.
   *
   ...
   * variable is usually the fully qualified name of the defining class of the static field. The tag
   * of the variable is used to separate variables which have the exact same group and name but
   * should not be shared (e.g., if the JVM loads different versions of the code and the types of
   * the variables have changed across those versions).
   *
   ...
   * Currently, this constructor requires that single class loader to be the bootstrap class loader.
   *
   ...
   *
   * <p>The users need to explicitly pass type {@code T} via a {@link TypeToken} or a {@link Class}
   * instance. This constructor takes a ({@code TypeToken} as it is more general (it can capture
   * complex types such as {@code Map<K, V>}). If the type is simple (can be represented fully by a
   * {@code Class} instance), the users can use some other constructor instead.
   *
   ...
   * @param initialValueSupplier the supplier that produces the initial value of the variable. It is
   *     called only when the variable is first created. The supplied value can be null.
   */
   ...
   * {@link #JvmWideVariable(String, String, String, TypeToken, Supplier)}. See the javadoc of that
   ...
   *       previous variable.
   *   <li>The type of the variable is a simple type represented by a {@link Class} instance instead
   *       of a {@link TypeToken} instance.
   *   <li>The initial value of the variable is provided directly instead of using a supplier.
   * </ol>
   *
   * <p>IMPORTANT: This constructor should be used only when the value of the variable has the exact
   * type {@code T}, not a sub-type of {@code T}; otherwise, type {@code T} alone would not be
   * sufficient to serve as a tag to capture the "uniqueness" of the variable.
   *
  ...
  public JvmWideVariable(
    ...
    this(
        ...
        name,
        collectComponentClasses(typeToken.getType()).stream()
            .map(Class::getName)
  ...
  private static ConcurrentMap<String, AtomicReference<Object>> createVariableTableIfNotExists() {
    ...
    String tag =
        collectComponentClasses(type).stream().map(Class::getName).collect(Collectors.joining("-"));
    ...
    synchronized (server) {
      if (!server.isRegistered(objectName)) {
        ValueWrapper<ConcurrentMap<String, AtomicReference<Object>>> valueWrapper =
            new ValueWrapper<>(new ConcurrentHashMap<String, AtomicReference<Object>>());
        try {
  ...
  /**
   * Returns the actual JVM-wide variable (an {@link AtomicReference} holding the variable's value).
   *
   ...
   */
  @Nullable
  public <V> V executeCallableSynchronously(@NonNull Callable<V> action) throws ExecutionException {
   ...
   * threads (e.g., at the end of a test method, when no other test is running). Otherwise, it would
   * break the thread safety of this class.
   */
   ...
   *
   * <ol>
   *   <li>The name of the class where the object is used. Note that we use the class name, not the
   *       class instance, so that the key is shared across class loaders.
   ...
   * @param keyType the type of the user-specified key, which must be loaded by the bootstrap class
   *     loader
