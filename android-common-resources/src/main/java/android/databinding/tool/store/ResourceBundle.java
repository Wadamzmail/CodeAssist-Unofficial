...
public class ResourceBundle implements Serializable {
  ...
  private static String qualifyViewNodeName(String viewNodeName) {
    switch (viewNodeName) {
      ...
      default:
        return viewNodeName.indexOf('.') == -1 ? "android.widget." + viewNodeName : viewNodeName;
    }
  }
  ...
  /**
   * Layout files that exist in the current build but do not contain data binding constructs. These
   * include: 1. Layout files that previously contained data binding constructs but are now no
   * longer containing them. We track these files to delete stale layout info files that data
   * binding generated in the previous build (see bug 153711619). 2. Layout files that do not have a
   * history or did not have data binding constructs in the previous build. We don't really need to
   * track these files as there are no corresponding stale layout info files to delete, but it's
   * easier (and okay) to leave them here.
   */
  ...
  public void addLayoutBundle(@NonNull LayoutFileBundle bundle, boolean fromSource) {
    ...
    List<LayoutFileBundle> bundles =
        mLayoutBundles.computeIfAbsent(bundle.mFileName, ignored -> new ArrayList<>());
  ...
  /**
   * @deprecated Use {@link #getAllLayoutFileBundlesInSource()} which contains {@link
   *     LayoutFileBundle}s for all layouts instead of just data binding. If you only care about
   *     data binding layouts, check {@link LayoutFileBundle#isBindingData()}.
   */
  @Deprecated
  ...
  /**
   * @deprecated Use {@link #getAllLayoutBundles()} which contains {@link LayoutFileBundle}s for all
   *     layouts instead of just data binding. If you only care about data binding layouts, check
  ...
  /**
   * Loads the class info from a folder. This log has the list of classes which are generated in
   * previous steps.
   */
  public static GenClassInfoLog loadClassInfoFromFolder(File folder) throws IOException {
    ...
    if (folder.isFile()) { // bazel
            ...
            .forEach(
                zipEntry -> {
                  if (zipEntry.getName().endsWith(DataBindingBuilder.BINDING_CLASS_LIST_SUFFIX)) {
                    try {
                      merged.addAll(
                          GenClassInfoLog.fromInputStream(zipFile.getInputStream(zipEntry)));
                    } catch (IOException e) {
      ...
      SuffixFileFilter fileFilter =
          new SuffixFileFilter(DataBindingBuilder.BINDING_CLASS_LIST_SUFFIX, IOCase.SYSTEM);
      Collection<File> files = FileUtils.listFiles(folder, fileFilter, TrueFileFilter.INSTANCE);
      for (File file : files) {
  ...
  private void validateBindingTargetIds() {
    mLayoutBundles.forEach(
        (name, layoutFileBundles) -> {
          layoutFileBundles.forEach(
              bundle -> {
                    ...
                    .forEach(
                        bindingTarget -> {
                          ...
                          if (id != null && conflictingIds.contains(id)) {
                            ...
                            String error =
                                String.format(ErrorMessages.DUPLICATE_VIEW_OR_INCLUDE_ID, tag, id);
  ...
  private void validateMultiResLayouts() {
        ...
        for (BindingTargetBundle target : layoutFileBundle.getBindingTargetBundles()) {
          if (target.isBinder()) {
            ...
            if (boundTo != null && !boundTo.isEmpty()) {
              ...
              IncludedLayout included =
                  mDependencyBinders.getOrDefault(target.getIncludedLayout(), null);
              if (included != null) {
            ...
            if (targetBinding == null) {
              L.d(
                  "There is no binding for %s, reverting to plain layout",
    ...
    for (Map.Entry<String, List<LayoutFileBundle>> bundles : mLayoutBundles.entrySet()) {
      ...
      Map<String, NameTypeLocation> importTypes =
          validateAndMergeNameTypeLocations(
              bundles.getValue(),
      ...
      for (LayoutFileBundle bundle : bundles.getValue()) {
        ...
        for (Map.Entry<String, NameTypeLocation> variable : variableTypes.entrySet()) {
          if (!NameTypeLocation.contains(bundle.mVariables, variable.getKey())) {
            ...
            L.d(
                "adding missing variable %s to %s / %s",
                variable.getKey(), bundle.mFileName, bundle.mConfigName);
          }
        }
        for (Map.Entry<String, NameTypeLocation> userImport : importTypes.entrySet()) {
          if (!NameTypeLocation.contains(bundle.mImports, userImport.getKey())) {
            ...
            L.d(
                "adding missing import %s to %s / %s",
                userImport.getKey(), bundle.mFileName, bundle.mConfigName);
          }
        }
      }
        ...
        try {
          ...
          for (BindingTargetBundle target : bundle.mBindingTargetBundles) {
            try {
              ...
              L.d(
                  "checking %s %s %s",
                  target.getId(), target.getFullClassName(), target.isBinder());
              if (target.mId != null) {
                if (target.isBinder()) {
        ...
        for (LayoutFileBundle bundle : bundles.getValue()) {
          for (BindingTargetBundle target : bundle.mBindingTargetBundles) {
            if (conflictingIds.contains(target.mId)) {
              Scope.registerError(
                  String.format(ErrorMessages.MULTI_CONFIG_ID_USED_AS_IMPORT, target.mId),
                  bundle,
                  target);
            }
          }
            ...
            if (target == null) {
              ...
              if (include == null) {
                bundle.createBindingTarget(
                    viewType.getKey(), viewType.getValue(), false, null, null, null);
              } else {
                BindingTargetBundle bindingTargetBundle =
                    bundle.createBindingTarget(viewType.getKey(), null, false, null, null, null);
                bindingTargetBundle.setIncludedLayout(include);
                bindingTargetBundle.setInterfaceType(viewType.getValue());
              }
            } else {
              L.d(
                  "setting interface type on %s (%s) as %s",
                  target.mId, target.getFullClassName(), viewType.getValue());
      ...
      for (LayoutFileBundle bundle : bundles) {
        ...
        Scope.registerError(
            String.format(
                ...
                found.name,
                found.type,
                bundle.mDirectory + "/" + bundle.getFileName()),
            bundle,
    ...
    for (LayoutFileBundle bundle : bundles) {
      Scope.registerError(
          String.format(
              ...
              bundle.getFullBindingClass(),
              bundle.mDirectory + "/" + bundle.getFileName()),
          bundle,
          bundle.getClassNameLocationProvider());
    }
    return sharedClassName;
  }
  ...
  public static class LayoutFileBundle implements Serializable, FileScopeProvider {
    ...
    /**
     * Updates configuration fields from the given bundle but does not change variables, binding
     * expressions etc.
     */
    ...
    public LayoutFileBundle(
        @NonNull RelativizableFile file,
        @NonNull String fileName,
        @NonNull String directory,
        @NonNull String modulePackage,
        boolean isMerge,
        boolean isBindingData,
        @NonNull String rootViewType,
    ...
    public LocationScopeProvider getClassNameLocationProvider() {
      if (mClassNameLocationProvider == null
          && mClassNameLocation != null
    ...
    public void addVariable(String name, String type, Location location, boolean declared) {
      Preconditions.check(
          ...
          "Cannot use same variable name twice. %s in %s",
          name,
          location);
    ...
    public void addImport(String alias, String type, Location location) {
      Preconditions.check(
          !NameTypeLocation.contains(mImports, alias),
          "Cannot import same alias twice. %s in %s",
          alias,
          location);
      mImports.add(new NameTypeLocation(alias, type, location));
    }

    public BindingTargetBundle createBindingTarget(
        String id,
        String viewName,
        boolean used,
        String tag,
        String originalTag,
        Location location) {
      BindingTargetBundle target =
          new BindingTargetBundle(id, viewName, used, tag, originalTag, location);
      ...
      if (mFullBindingClass == null) {
        if (mBindingClass == null) {
          mFullBindingClass =
              getModulePackage()
                  + ".databinding."
                  + ParserHelper.toClassName(getFileName())
                  + "Binding";
  ...
  public static class BindingTargetBundle implements Serializable, LocationScopeProvider {
    ...
    public void addBinding(
        String name, String expr, boolean isTwoWay, Location location, Location valueLocation) {
    ...
    public boolean isBinder() {
      return mIncludedLayout != null
          && (!"android.view.View".equals(mInterfaceType)
    ...
    public String getFullClassName() {
      ...
      if (mFullClassName == null) {
        L.e(
            "Unexpected full class name = null. view = %s, interface = %s, layout = %s",
  ...
  public static class IncludedLayout {
    ...
    private static class Builder {
      ...
      public IncludedLayout build() {
        return new IncludedLayout(mLayoutName, mModulePackage, mInterfaceQName);
      }
    }
  }
}
