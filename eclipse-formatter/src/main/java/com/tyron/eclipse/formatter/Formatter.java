...
public class Formatter {
  ...
  /**
   * Formats the given source using the default java convention options Returns the original string
   * if the source cannot be formatted.
   *
  ...
  public static String format(
      String source, int indentLevel, int start, int length, DefaultCodeFormatterOptions options) {
    DefaultCodeFormatter formatter = new DefaultCodeFormatter(options);
    TextEdit format =
        formatter.format(
            DefaultCodeFormatter.K_COMPILATION_UNIT, source, start, length, indentLevel, "\n");
