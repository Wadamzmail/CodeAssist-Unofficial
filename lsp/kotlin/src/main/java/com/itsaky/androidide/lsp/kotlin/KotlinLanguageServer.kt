...
class KotlinLanguageServer : ILanguageServer {
    ...
    override fun setupWorkspace(workspace: JavaModule) {
        ...
        if (!initialized) {
            ...
            android.util.Log.d("Kotlin LSP initialized with stdlib index")
            initialized = true
        }
    ...
    private fun indexClasspaths(workspace: JavaModule) {
        android.util.Log.d("indexClasspaths called, subProjects count={}", workspace.getSubprojects().size)
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                for (project in workspace.getSubprojects()) {
                    android.util.Log.d("Checking project: {} (type={})", project.moduleName, project::class.simpleName)
                    if (project is JavaModule) {
                        val projectClasspaths = project.getLibraries().toMutableList()
                        android.util.Log.d("Project {} has {} classpath entries", project.moduleName, projectClasspaths.size)
                ...
                android.util.Log.d("Total classpath entries found: {} (including {} boot classpaths)", classpaths.size, bootClasspaths.size)
                if (classpaths.isNotEmpty()) {
                    val files = classpaths.filter { it.exists() }
                    android.util.Log.d("Indexing {} existing classpath entries for Kotlin LSP", files.size)
                    ktLspServer.setClasspathAsync(files).thenAccept { index ->
                        android.util.Log.d("Kotlin LSP classpath indexed: {} symbols from {} jars", index.size, index.jarCount)
                    }.exceptionally { e ->
                        android.util.Log.d("Error in classpath indexing async", e)
                        null
                    }
                } else {
                    android.util.Log.d("No classpath entries found for Kotlin LSP")
                }
            } catch (e: Exception) {
                android.util.Log.d("Error indexing classpaths for Kotlin LSP", e)
                print("Error indexing classpaths for Kotlin LSP"+e)
            }
        }
    }
