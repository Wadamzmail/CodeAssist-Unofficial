...
public class ResourceMerger extends DataMerger<ResourceMergerItem, ResourceFile, ResourceSet> {
  ...
  /**
   * Cached resource keys that should not be in the final artifact. If null, then the cache will
   * have to be recomputed. To clear the cache, use {@link #clearFilterCache()}.
   */
  @Nullable private Set<String> mRejectCache;
   ...
   * items). The first map key is the associated qualifier for the items, the second map key is the
   * item name.
   */
  protected final Map<String, Map<String, ResourceMergerItem>> mMergedItems = new HashMap<>();

  /**
   * Reads the {@link ResourceSet} from the blob XML. {@link ResourceMerger} deals with two kinds of
   * sets - {@link GeneratedResourceSet} and "plain" {@link ResourceSet} . Instances of the former
   * are marked with {@code generated="true"} attribute. Instances of the latter have a {@code
   * generated-set} attribute that references the corresponding generated set by name. For any
   * variant, the generated set has a lower priority, so it comes in the XML first. This means we
   * will find it by name at this stage.
   */
  @Override
  protected ResourceSet createFromXml(Node node, @Nullable String aaptEnv) throws MergingException {
    String generated = NodeUtils.getAttribute(node, GeneratedResourceSet.ATTR_GENERATED);

    ResourceNamespace aaptNamespace = null;
    String namespaceUri = NodeUtils.getAttribute(node, GeneratedResourceSet.ATTR_AAPT_NAMESPACE);
    ...
    try {
      if (touched || (previouslyWrittenItem == null && !removed)) {
        ...
        if (previouslyWrittenItem == null
            || !NodeUtils.compareElementNode(
                newItem.getValue(), previouslyWrittenItem.getValue(), false)) {
          newItem.setTouched();
        }
  ...
  @Override
  protected void loadAdditionalData(@NonNull Node mergedItemsNode, boolean incrementalState)
      throws MergingException {
    ...
    for (int j = 0, n2 = configurationList.getLength(); j < n2; j++) {
      ...
      Attr qualifierAttr = (Attr) configuration.getAttributes().getNamedItem(ATTR_QUALIFIER);
  ...
  protected void writeAdditionalData(Document document, Node rootNode) {
    ...
    for (String qualifier : mMergedItems.keySet()) {
      ...
      NodeUtils.addAttribute(document, qualifierNode, null, ATTR_QUALIFIER, qualifier);
