...
public class AttributeValueUtils {
  ...
  public static void addValueItems(
    ...
    AttrResourceValue attribute =
        AttributeProcessingUtil.getLayoutAttributeFromNode(
            repository, attr.getOwnerElement(), attr.getLocalName(), attrNamespace);
    if (attribute == null) {
  ...
  private static void addValues(
    ...
    if (prefix.startsWith("@")) {
      ...
      ResourceNamespace.Resolver resolver = DOMUtils.getNamespaceResolver(attr.getOwnerDocument());
      ...
      List<ResourceValue> items =
          repository.getResources(namespace, fromTag).asMap().values().stream()
      ...
      for (ResourceValue value : items) {
        if (value.getResourceType().getName().startsWith(resourceType)) {
          String label =
              value.asReference().getRelativeResourceUrl(appNamespace, resolver).toString();
