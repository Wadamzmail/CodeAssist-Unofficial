 ...
 * obtained from R.txt instead, when it is available. This means that {@link
...
public class AarSourceResourceRepository extends AbstractAarResourceRepository {
  /**
   * Increment when making changes that may affect content of repository cache files. Used together
   * with CachingData.codeVersion. Important for developer builds.
   */
  ...
  /**
   * Protocol used for constructing {@link PathString}s returned by the {@link
   * BasicFileResourceItem#getSource()} method.
   */
  ...
  protected AarSourceResourceRepository(
      @NotNull RepositoryLoader<? extends AarSourceResourceRepository> loader,
    ...
    myManifestPackageName =
        NullableLazyValue.lazyNullable(
            () -> {
              try {
   ...
   */
  @NotNull
  public static AarSourceResourceRepository create(
      @NotNull Path resourceDirectoryOrFile, @NotNull String libraryName) {
  ...
  public static AarSourceResourceRepository create(
    ...
    return create(
        resourceDirectoryOrFile, null, ResourceNamespace.RES_AUTO, libraryName, cachingData);
  }
   ...
   * AarResourceRepositoryCache.getSourceRepository instead of this method.
   *
   * @param resourceFolderRoot specifies the resource files to be loaded. The list of files to be
   *     loaded can be restricted by providing a not null {@code resourceFolderResources} list of
   *     files and subdirectories that should be loaded.
   * @param resourceFolderResources A null value indicates that all files and subdirectories in
   *     {@code resourceFolderRoot} should be loaded. Otherwise files and subdirectories specified
   *     in {@code resourceFolderResources} are loaded.
   * @param libraryName the name of the library
   * @param cachingData data used to validate and create a persistent cache file
   * @return the created resource repository
   */
  @NotNull
  public static AarSourceResourceRepository create(
      @NotNull PathString resourceFolderRoot,
    ...
    return create(
        resDir, resourceFolderResources, ResourceNamespace.RES_AUTO, libraryName, cachingData);
  }

  @NotNull
  private static AarSourceResourceRepository create(
      @NotNull Path resourceDirectoryOrFile,
    ...
    if (cachingData != null
        ...
        && repository.loadFromPersistentCache(cachingData)) {
      return repository;
    }
   ...
   *
   * @return true if the repository was loaded from the cache, or false if the cache does not exist
   *     or is out of date
  ...
  private boolean loadFromPersistentCache(@NotNull CachingData cachingData) {
    byte[] header =
        ResourceSerializationUtil.getCacheFileHeader(
            stream -> writeCacheHeaderContent(cachingData, stream));
  ...
  private void createPersistentCache(@NotNull CachingData cachingData) {
    ...
    ResourceSerializationUtil.createPersistentCache(
        cachingData.getCacheFile(), header, stream -> writeToStream(stream, config -> true));
  }

  protected void writeCacheHeaderContent(
      @NotNull CachingData cachingData, @NotNull Base128OutputStream stream) throws IOException {
  ...
  /**
   * Called when an attempt to load from persistent cache fails after some data may have already
   * been loaded.
   */
   ...
   */
  void writeToStream(
      @NotNull Base128OutputStream stream, @NotNull Predicate<FolderConfiguration> configFilter)
      throws IOException {
  ...
  protected void loadFromStream(
    ...
    ResourceSerializationUtil.readResourcesFromStream(
        stream, stringCache, namespaceResolverCache, this, this::addResourceItem);
  }
  ...
  private static class Loader extends RepositoryLoader<AarSourceResourceRepository> {
      ...
      if (myZipFile == null) {
        ...
        if (zipEntry != null) {
          try (BufferedReader reader =
              new BufferedReader(
                  new InputStreamReader(myZipFile.getInputStream(zipEntry), UTF_8))) {
            SymbolTable symbolTable =
                SymbolIo.readFromAaptNoValues(
                    reader, FN_RESOURCE_TEXT + " in " + myResourceDirectoryOrFile, null);
            myRTxtIds = computeIds(symbolTable);
            return true;
          } catch (ProcessCanceledException e) {
            throw e;
            ...
            LOG.warning(
                "Failed to load id resources from "
                    + FN_RESOURCE_TEXT
                    + " in "
                    + myResourceDirectoryOrFile
                    + " "
    ...
    private void createResourcesForRTxtIds(@NotNull AarSourceResourceRepository repository) {
      if (!myRTxtIds.isEmpty()) {
        RepositoryConfiguration configuration =
            getConfiguration(repository, ResourceItem.DEFAULT_CONFIGURATION);
    ...
    protected void loadPublicResourceNames() {
      if (myZipFile == null) {
        ...
        try (BufferedReader reader = Files.newBufferedReader(file)) {
          ...
          myDefaultVisibility =
              ResourceVisibility
                  .PUBLIC; // The "public.txt" file does not exist - myDefaultVisibility will be
          // PUBLIC.
        } catch (IOException e) {
        ...
        if (zipEntry == null) {
          ...
          try (BufferedReader reader =
            ...
            // Failure to load public resource names is not considered fatal.
            LOG.warning(
                "Error reading " + FN_PUBLIC_TXT + " from " + myResourceDirectoryOrFile + " " + e);
          }
        }
      }
    }
    ...
    protected void addResourceItem(
        @NotNull BasicResourceItem item, @NotNull AarSourceResourceRepository repository) {
