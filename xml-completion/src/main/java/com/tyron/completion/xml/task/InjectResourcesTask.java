...
/**
 * Used to create fake R.java files from the project resources for it to show up on code completion.
 * Files generated from this task should not be included in the compilation process as the values of
 * the fields are not accurate from what AAPT2 generates.
 */
public class InjectResourcesTask {
  ...
  public static void inject(@NonNull Project project, @NonNull AndroidModule module)
      throws IOException {
    ...
    for (com.android.ide.common.rendering.api.ResourceNamespace namespace : namespaces) {
      ...
      for (ResourceType resourceType : resourceTypes) {
        ...
        for (Map.Entry<String, ResourceItem> resourceItemEntry : resources.entries()) {
          addResource(id, namespace, symbols, resourceType, resourceItemEntry);
          id++;
        }
      }
    }
  ...
  private void addResource(
    ...
    SymbolLoader.SymbolEntry entry =
        new SymbolLoader.SymbolEntry(replacedName, getType(resourceType), String.valueOf(id));
    symbols.put(resourceType.getName(), replacedName, entry);
  }

  private void addStyleableResource(
      int id,
      ResourceNamespace namespace,
      Table<String, String, SymbolLoader.SymbolEntry> symbols,
      Map.Entry<String, ResourceItem> resourceItemEntry) {
    ...
    String replacedName = convertName(value.getName());
    SymbolLoader.SymbolEntry entry = new SymbolLoader.SymbolEntry(replacedName, "int[]", valueItem);
