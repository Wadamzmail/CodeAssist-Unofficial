...
public class AndroidAttributeUtils {
  ...
  public static void addLayoutAttributes(
    ...
    if (parentNode != null) {
      Function<String, Set<String>> provider =
          tag ->
              ImmutableSet.of(
                  tag, getLayoutStyleablePrimary(tag), getLayoutStyleableSecondary(tag));
  ...
  private static void addNamespaceAttributes(
      CompletionList.Builder builder, DOMElement rootElement, ResourceNamespace.Resolver resolver) {
    if (resolver.uriToPrefix(ResourceNamespace.ANDROID.getXmlNamespaceUri()) == null) {
      CompletionItem completionItem =
          CompletionItem.create("androidNs", "Namespace", "xmlns:android", DrawableKind.Attribute);
      completionItem.addFilterText("android");
      completionItem.setInsertHandler(
          new AttributeInsertHandler(
              ResourceNamespace.ANDROID.getXmlNamespaceUri(), completionItem));
      builder.addItem(completionItem);
    }

    if (resolver.uriToPrefix(ResourceNamespace.RES_AUTO.getXmlNamespaceUri()) == null) {
      CompletionItem completionItem =
          CompletionItem.create("appNs", "Namespace", "xmlns:app", DrawableKind.Attribute);
      completionItem.addFilterText("app");
      completionItem.setInsertHandler(
          new AttributeInsertHandler(
              ResourceNamespace.RES_AUTO.getXmlNamespaceUri(), completionItem));
      builder.addItem(completionItem);
    }
  }

  private static void addAttributes(
      List<AttrResourceValue> tagAttributes, DOMNode node, CompletionList.Builder builder) {
    ...
    for (AttrResourceValue tagAttribute : tagAttributes) {
      ...
      if (name.contains(":")) {
        ...
        ResourceNamespace namespace =
            ResourceNamespace.fromNamespacePrefix(
                prefix, tagAttribute.getNamespace(), tagAttribute.getNamespaceResolver());
        reference = new ResourceReference(namespace, tagAttribute.getResourceType(), fixedName);
      } else {
        reference = tagAttribute.asReference();
      }
      String prefix = resolver.uriToPrefix(reference.getNamespace().getXmlNamespaceUri());
      ...
      String commitText =
          TextUtils.isEmpty(prefix) ? reference.getName() : prefix + ":" + reference.getName();
      if (uniques.contains(commitText)) {
        continue;
      }

      CompletionItem attribute =
          CompletionItem.create(commitText, "Attribute", commitText, DrawableKind.Attribute);
