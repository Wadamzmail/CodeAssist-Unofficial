...
public class FileTimeStampLengthHasher {
  ...
  /**
   * Computes a combined 64-bit hash of time stamp and size of a {@link VirtualFile}. Returns an
   * array of 8 zero bytes if the virtual file is null or is not valid.
   */
  @NotNull
  public static byte[] hash(@Nullable File virtualFile) {
    ...
    return Hashing.sipHash24()
        .newHasher()
        .putLong(virtualFile.lastModified())
        .putLong(virtualFile.length())
        .hash()
        .asBytes();
  }
}
