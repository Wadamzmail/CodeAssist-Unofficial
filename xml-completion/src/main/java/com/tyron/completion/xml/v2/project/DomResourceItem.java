...
public class DomResourceItem implements ResourceItem {
  ...
  /**
   * This weak reference is kept exclusively for the {@link #wasTag(DOMElement)} method. Once the
   * original tag is garbage collected, the {@link #wasTag(DOMElement)} method will return false for
   * any tag except the one pointed to by {@link #myTagPointer}.
   */
  @Nullable private final WeakReference<DOMElement> myOriginalTag;

  private final File file;

  private DomResourceItem(
      @NotNull String name,
   ...
   */
  @NotNull
  public static DomResourceItem forXmlTag(
      @NotNull String name,
      @NotNull ResourceType type,
      @NotNull ResourceFolderRepository owner,
   ...
   */
  @NotNull
  public static DomResourceItem forFile(
      @NotNull String name,
      @NotNull ResourceType type,
      @NotNull ResourceFolderRepository owner,
      @NotNull File file) {
  ...
  public ResourceValue getResourceValue() {
    if (myResourceValue == null) {
      ...
      if (tag == null) {
        ...
        Density density =
            type == ResourceType.DRAWABLE || type == ResourceType.MIPMAP
                ? getFolderDensity()
                : null;
        ...
        if (density != null) {
          myResourceValue =
              new DensityBasedResourceValueImpl(
                  getNamespace(), myType, myName, path, density, null);
        } else {
  ...
  private ResourceValue parseXmlToResourceValue(@Nullable DOMElement tag) {
    ...
    switch (myType) {
      case STYLE:
        ...
        value =
            parseStyleValue(tag, new StyleResourceValueImpl(getNamespace(), myName, parent, null));
        break;
      case STYLEABLE:
        value =
            parseDeclareStyleable(
                tag, new StyleableResourceValueImpl(getNamespace(), myName, null, null));
        break;
      case ATTR:
      ...
      case ARRAY:
        value =
            parseArrayValue(
                tag,
                new ArrayResourceValueImpl(getNamespace(), myName, null) {
      ...
      case PLURALS:
        value =
            parsePluralsValue(
                tag,
                new PluralsResourceValueImpl(getNamespace(), myName, null, null) {
      ...
      case STRING:
        value =
            parseTextValue(tag, new DomTextResourceValue(getNamespace(), myName, null, null, null));
        break;
      default:
        value = parseValue(tag, new ResourceValueImpl(getNamespace(), myType, myName, null));
        break;
    }
    value.setNamespaceResolver(
        namespacePrefix ->
            DOMUtils.getNamespaceResolver(tag.getOwnerDocument()).prefixToUri(namespacePrefix));
    return value;
  }

  @NotNull
  private static StyleResourceValueImpl parseStyleValue(
      @NotNull DOMElement tag, @NotNull StyleResourceValueImpl styleValue) {
      ...
      if (!StringUtil.isEmpty(name)) {
        ...
        StyleItemResourceValueImpl itemValue =
            new StyleItemResourceValueImpl(
                styleValue.getNamespace(), name, value, styleValue.getLibraryName());
        itemValue.setNamespaceResolver(
            namespacePrefix ->
                DOMUtils.getNamespaceResolver(tag.getOwnerDocument()).prefixToUri(namespacePrefix));
  ...
  private StyleableResourceValueImpl parseDeclareStyleable(
      @NotNull DOMElement tag, @NotNull StyleableResourceValueImpl declareStyleable) {
    for (DOMElement child : DOMUtils.getSubTags(tag)) {
      String name = getAttributeValue(child, ATTR_NAME);
      if (!StringUtil.isEmpty(name)) {
        ...
        if (url != null) {
          ResourceReference resolvedAttr =
              url.resolve(
                  getNamespace(),
                  namespacePrefix ->
                      DOMUtils.getNamespaceResolver(tag.getOwnerDocument())
                          .prefixToUri(namespacePrefix));
          if (resolvedAttr != null) {
            AttrResourceValue attr =
                parseAttrValue(child, new AttrResourceValueImpl(resolvedAttr, null));
  ...
  private static AttrResourceValueImpl parseAttrValue(
      @NotNull DOMElement attrTag, @NotNull AttrResourceValueImpl attrValue) {
  ...
  @NotNull
  private static ArrayResourceValueImpl parseArrayValue(
      @NotNull DOMElement tag, @NotNull ArrayResourceValueImpl arrayValue) {
    for (DOMElement child : DOMUtils.getSubTags(tag)) {
  ...
  @NotNull
  private static PluralsResourceValueImpl parsePluralsValue(
      @NotNull DOMElement tag, @NotNull PluralsResourceValueImpl value) {
    for (DOMElement child : DOMUtils.getSubTags(tag)) {
  ...
  @NotNull
  private static ResourceValueImpl parseValue(
      @NotNull DOMElement tag, @NotNull ResourceValueImpl value) {
  ...
  @NotNull
  private static DomTextResourceValue parseTextValue(
      @NotNull DOMElement tag, @NotNull DomTextResourceValue value) {
    String text = tag.getTextContent();
    text = ValueXmlHelper.unescapeResourceString(text, true, true);
    value.setValue(text);
  ...
  public DomResourceFile getSourceFile() {
    ...
    return new DomResourceFile(
        file,
        ImmutableList.of(this),
        folderType,
        new RepositoryConfiguration(myOwner, folderConfiguration));
  }
