...
public class ModuleResourceRepository extends MultiResourceRepository
   ...
   * modules.
   *
   * <p>The returned repository needs to be registered with a {@link
   ...
   */
  @NotNull
  static ModuleResourceRepository forMainResources(
      @NotNull AndroidModule facet, @NotNull ResourceNamespace namespace) {
    List<File> resourceDirectories =
        facet.getContentRoots().stream()
    ...
    List<LocalResourceRepository> childRepositories =
        new ArrayList<>(1 + resourceDirectories.size());
  ...
  /**
   * Inserts repositories for the given {@code resourceDirectories} into {@code childRepositories},
   * in the right order.
   *
   * <p>{@code resourceDirectories} is assumed to be in the order returned from {@link
   * SourceProviderManager#getCurrentSourceProviders()}, which is the inverse of what we need. The
   * code in {@link MultiResourceRepository#getMap(ResourceNamespace, ResourceType, boolean)} gives
   * priority to child repositories which are earlier in the list, so after creating repositories
   * for every folder, we add them in reverse to the list.
   *
   ...
   */
  private static void addRepositoriesInReverseOverlayOrder(
      @NotNull List<File> resourceDirectories,
    ...
    for (int i = resourceDirectories.size(); --i >= 0; ) {
      ...
      ResourceFolderRepository resourceFolderRepository =
          ResourceFolderRepository.create(
              facet,
              resourceDirectory,
  ...
  private ModuleResourceRepository(
      @NotNull AndroidModule facet,
  ...
  public String getPackageName() {
    ...
    return packageName != null ? packageName : myFacet.getPackageName();
  }

  @NotNull
  @Override
  public String toString() {
    return MoreObjects.toStringHelper(this).addValue(mySourceSet).toString();
  }
}
