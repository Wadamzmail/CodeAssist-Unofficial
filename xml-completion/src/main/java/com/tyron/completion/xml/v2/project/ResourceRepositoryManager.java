...
public class ResourceRepositoryManager {
  ...
  private static final Key<ResourceRepositoryManager> KEY =
      Key.create(ResourceRepositoryManager.class.getName());
  ...
  /**
   * If the module is namespaced, this is the shared {@link ResourceNamespace} instance
   * corresponding to the package name from the manifest.
   */
  ...
  public static ResourceRepositoryManager getInstance(@NotNull AndroidModule facet) {
    Namespacing namespacing =
        Strings.isNullOrEmpty(facet.getNamespace()) ? Namespacing.DISABLED : Namespacing.REQUIRED;
  ...
  private ResourceRepositoryManager(
      @NotNull AndroidModule facet, @NotNull Namespacing namespacing) {
    myFacet = facet;
   ...
   *
   * <p><b>Note:</b> This method should not be called on the event dispatch thread since it may take
   * long time, or block waiting for a read action lock.
   *
  ...
  /**
   * Returns the previously computed resource repository for a module along with all its (local)
   * module dependencies. The repository doesn't contain resources from AAR dependencies.
   *
   ...
   * folders). Does not include resources from any dependencies.
   *
   * <p><b>Note:</b> This method should not be called on the event dispatch thread since it may take
   * long time, or block waiting for a read action lock.
   *
  ...
  /**
   * Returns the previously computed resource repository for a single module (which can possibly
   * have multiple resource folders). Does not include resources from any dependencies.
   *
   * @return the repository, or null if the repository hasn't been created yet
   * @see #getModuleResources()
   */
  ...
  /**
   * Returns the repository with all non-framework resources available to a given module (in the
   * current variant). This includes not just the resources defined in this module, but in any other
   * modules that this module depends on, as well as any libraries those modules may depend on (e.g.
   * appcompat). This repository also contains sample data resources associated with the {@link
   * ResourceNamespace#TOOLS} namespace.
   *
   * <p>When a layout is rendered in the layout editor, it is getting resources from the app
   * resource repository: it should see all the resources just like the app does.
   *
   * <p><b>Note:</b> This method should not be called on the event dispatch thread since it may take
   * long time, or block waiting for a read action lock.
   *
  ...
  /**
   * Returns the previously computed repository with all non-framework resources available to a
   * given module (in the current variant). This includes not just the resources defined in this
   * module, but in any other modules that this module depends on, as well as any AARs those modules
   * depend on (e.g. appcompat). This repository also contains sample data resources associated with
  ...
  private Map<ExternalAndroidLibrary, AarResourceRepository> computeLibraryResourceMap() {
    List<CodeAssistAndroidLibrary> codeAssistLibraries =
        ((AndroidModuleImpl) myFacet)
            .getCodeAssistLibraries().stream()
                .filter(it -> it instanceof CodeAssistAndroidLibrary)
                .map(it -> (CodeAssistAndroidLibrary) it)
                .collect(Collectors.toList());
    AarResourceRepositoryCache aarResourceRepositoryCache =
        AarResourceRepositoryCache.getInstance();
    ...
    ImmutableMap.Builder<ExternalAndroidLibrary, AarResourceRepository> map =
        ImmutableMap.builder();
    for (Map.Entry<ExternalAndroidLibrary, Future<AarResourceRepository>> entry :
        futures.entrySet()) {
      try {
  ...
  public ResourceNamespace getNamespace() {
    ...
    if (mySharedNamespaceInstance == null
        || !namespace.equals(mySharedNamespaceInstance.getPackageName())) {
  ...
  private static class LocalesAndLanguages {
    ...
    LocalesAndLanguages(
        @NotNull ImmutableList<Locale> locales, @NotNull ImmutableSortedSet<String> languages) {
