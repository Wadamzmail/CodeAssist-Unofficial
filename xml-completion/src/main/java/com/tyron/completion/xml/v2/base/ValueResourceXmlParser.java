...
/**
 * XML pull parser for value resource files. Provides access the resource namespace resolver for the
 * current tag.
 */
class ValueResourceXmlParser extends CommentTrackingXmlPullParser {
  ...
  /**
   * Returns the namespace resolver for the current XML node. The parser has to be positioned on a
   * start tag when this method is called.
   */
  @NotNull
      ...
      case START_TAG:
        {
          ...
          NamespaceResolver current =
              parent != null && parent.getNamespaceCount() == namespaceCount
                  ? parent
                  : getOrCreateResolver();
